...
extends Component {
    ...
    public void render(MatrixStack stack, int mouseX, int mouseY) {
        ...
        for (IItem addingItem : Expensive.getInstance().getItemStorage().getItems()) {
            Scissor.push();
            Scissor.setFromComponentCoordinates(this.x + currentX - 100.0f, this.y + currentY, 320.0, 55.0);
            Item item = addingItem.getItem();
            String displayName = item.getName().getString();
            DisplayUtils.drawRoundedRect(this.x + currentX - 1.0f, this.y + currentY - 1.0f, 122.0f, 57.0f, new Vector4f(6.0f, 6.0f, 6.0f, 6.0f), new Vector4i(ColorUtils.getColor(0), ColorUtils.getColor(90), ColorUtils.getColor(180), ColorUtils.getColor(270)));
