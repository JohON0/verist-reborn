...
implements IMinecraft {
    public static Entity getMouseOver(Entity target, float yaw, float pitch, double distance) {
        ...
        if (entity2 != null) {
            if (Minecraft.world != null) {
                ...
                Vector3d startVec = entity2.getEyePosition(1.0f);
                ...
                EntityRayTraceResult entityraytraceresult = MouseUtil.rayTraceEntities(entity2, startVec, endVec, axisalignedbb = target.getBoundingBox().grow(target.getCollisionBorderSize()), p_lambda$getMouseOver$0_0_ -> !p_lambda$getMouseOver$0_0_.isSpectator() && p_lambda$getMouseOver$0_0_.canBeCollidedWith(), distance);
    ...
    public static EntityRayTraceResult rayTraceEntities(Entity shooter, Vector3d startVec, Vector3d endVec, AxisAlignedBB boundingBox, Predicate<Entity> filter, double distance) {
        ...
        return entity2 == null ? null : new EntityRayTraceResult(entity2, closestHitVec);
    }

    public static RayTraceResult rayTrace(double rayTraceDistance, float yaw, float pitch, Entity entity2) {
        Vector3d startVec = Minecraft.player.getEyePosition(1.0f);
        Vector3d directionVec = MouseUtil.getVectorForRotation(pitch, yaw);
        ...
        return Minecraft.world.rayTraceBlocks(new RayTraceContext(startVec, endVec, RayTraceContext.BlockMode.OUTLINE, RayTraceContext.FluidMode.NONE, entity2));
    }

    public static RayTraceResult rayTraceResult(double rayTraceDistance, float yaw, float pitch, Entity entity2) {
        RayTraceResult object = null;
        if (entity2 != null) {
            if (Minecraft.world != null) {
                float partialTicks = mc.getRenderPartialTicks();
                double distance = rayTraceDistance;
                object = MouseUtil.rayTrace(rayTraceDistance, yaw, pitch, entity2);
                Vector3d vector3d = entity2.getEyePosition(partialTicks);
                boolean flag = false;
                double d1 = distance;
                ...
                float f = 1.0f;
                AxisAlignedBB axisalignedbb = entity2.getBoundingBox().expand(vector3d1.scale(distance)).grow(1.0, 1.0, 1.0);
                EntityRayTraceResult entityraytraceresult = ProjectileHelper.rayTraceEntities(entity2, vector3d, vector3d2, axisalignedbb, p_lambda$getMouseOver$0_0_ -> !p_lambda$getMouseOver$0_0_.isSpectator() && p_lambda$getMouseOver$0_0_.canBeCollidedWith(), d1);
                if (entityraytraceresult != null) {
    ...
    public static boolean rayTraceWithBlock(double rayTraceDistance, float yaw, float pitch, Entity entity2, Entity target) {
        RayTraceResult object = null;
        if (entity2 != null) {
            if (Minecraft.world != null) {
                float partialTicks = mc.getRenderPartialTicks();
