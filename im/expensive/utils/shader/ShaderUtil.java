...
implements IMinecraft {
    ...
    String blur = "#version 120\n\n uniform sampler2D textureIn;\n uniform vec2 texelSize, direction;\n uniform float radius, weights[256];\n\n #define offset texelSize * direction\n\n void main() {\n     vec2 uv = gl_TexCoord[0].st;\n         uv.y = 1.0 - uv.y;\n     vec3 color = texture2D(textureIn, uv).rgb * weights[0];     float totalWeight = weights[0]; for (int i = 1; i <= 32; i++) {     if (i > int(radius)) break;     vec2 offsetUV = i * offset;     color += texture2D(textureIn, uv + offsetUV).rgb * weights[i];     color += texture2D(textureIn, uv - offsetUV).rgb * weights[i];     totalWeight += 2.0 * weights[i]; }     gl_FragColor = vec4(color / totalWeight, 1.0);}";
    ...
    public ShaderUtil(String fragmentShaderLoc) {
        try {
            int fragmentShaderID = switch (fragmentShaderLoc) {
                ...
                case "MainMenu" -> this.createShader(Shaders.getInstance().getMainmenu(), 35632);
