...
implements IMinecraft {
    ...
    public void renderTextWithHorizontalGradient(MatrixStack ms, String text, float x, float y, int color, float fontSize, int startColor, int endColor) {
        float textWidth = Fonts.sfui.getWidth(text, fontSize);
        float posX = x;
        float posY = y;
        for (int i = 0; i < text.length(); ++i) {
            char character = text.charAt(i);
            float charWidth = Fonts.sfui.getWidth(String.valueOf(character), fontSize);
            float gradientFactor = (float)i / (float)text.length();
            int gradientColor = ColorUtils.gradient(-1, ColorUtils.getColor(90), 10, 10);
            Fonts.sfui.drawText(ms, String.valueOf(character), posX, posY, color, fontSize);
            posX += charWidth;
        }
    }
    ...
    public void drawGradientText(MatrixStack stack, String text, float x, float y, int startColor, int endColor, int fontSize, float shadowOffset) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            Fonts.montserrat.drawText(stack, String.valueOf(character), currentX, y, color, fontSize, shadowOffset);
            currentX += Fonts.montserrat.getWidth(String.valueOf(character), (float)fontSize) - 1.0f;
        }
    }

    public void drawGradientText1(MatrixStack stack, String text, float x, float y, int startColor, int endColor, int fontSize, float shadowOffset) {
        float gradientStep = 1.0f / (float)text.length();
        float currentX = x;
        for (int i = 0; i < text.length(); ++i) {
            char character = text.charAt(i);
