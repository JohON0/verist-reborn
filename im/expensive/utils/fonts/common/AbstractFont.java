 ...
 */
package im.expensive.utils.fonts.common;

import com.mojang.blaze3d.platform.GlStateManager;
import im.expensive.utils.fonts.Fonts;
import im.expensive.utils.fonts.other.TextureHelper;
import im.expensive.utils.fonts.other.Wrapper;
...
implements Wrapper {
    ...
    public static final Font getFont(String fileName, int style, int size) {
        String path = "/assets/minecraft/expensive/font2/".concat(fileName);
        Font font = null;
        try {
            font = Font.createFont(0, Objects.requireNonNull(Fonts.class.getResourceAsStream(path))).deriveFont(style, size);
    ...
    public static final Font getFontWindows(String fileName, int style, int size) {
        ...
        Font font = null;
        try {
            font = Font.createFont(0, Files.newInputStream(new File("C:/Windows/Fonts/" + fileName).toPath(), new OpenOption[0])).deriveFont(style, size);
        } catch (FontFormatException | IOException e) {
            e.printStackTrace();
        }
        return font;
