...
extends Function {
    ...
    private void handle(boolean all) {
        ...
        synchronized (list) {
            ...
            while (iterator2.hasNext()) {
                ...
                double factor = Minecraft.player.getPositionVec().distanceTo(this.realPos) / (double)((Float)this.range.get()).floatValue();
                if (!all && (double)packetData.timestamp() + (double)((Float)this.delay.get()).longValue() * factor > (double)System.currentTimeMillis()) break;
                try {
                    NetworkManager.processPacket(packetData.packet(), Minecraft.player.connection);
    ...
    private boolean shouldLagging() {
        ...
        if (this.target.isInvulnerable()) return false;
        if (!(Minecraft.player.getPositionVec().distanceTo(this.realPos) <= (double)((Float)this.range.get()).floatValue())) return false;
        return true;
    }
