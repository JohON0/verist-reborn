...
extends Function {
    ...
    private void onRender(WorldEvent worldEvent) {
        if (this.mod.is("Nimb")) {
            ...
            Vector3d vector3d = MathUtil.interpolate(Minecraft.player.getPositionVec(), new Vector3d(Minecraft.player.lastTickPosX, Minecraft.player.lastTickPosY, Minecraft.player.lastTickPosZ), worldEvent.getPartialTicks());
            vector3d.y += (double)0.1f;
            RenderSystem.translated(vector3d.x, vector3d.y + (double)Minecraft.player.getHeight(), vector3d.z);
            ...
            for (int i = 0; i <= 360; ++i) {
                ...
                bufferBuilder.pos(f2, vector3d.y + (double)Minecraft.player.getHeight(), f3).color(ColorUtils.setAlpha(HUD.getColor(i, 1.0f), 255)).endVertex();
            }
